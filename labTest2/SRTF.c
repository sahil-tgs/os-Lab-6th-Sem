...
void SRTF(Process *process, int n, FILE *out) {
    int currentTime = 0;
    int completed = 0;
    int prev = 0;
    int min = INT_MAX;
    ...
    while (completed != n) {
        for (int i = 0; i < n; i++) {
            if ((process[i].arrivalTime <= currentTime) && (process[i].remainingTime < min) && process[i].remainingTime > 0) {
        ...
        } else if (process[shortest].remainingTime % process[shortest].ioAfter == 0) {
            fprintf(out,"%d %s I/O ", currentTime + 1, process[shortest].name);
            currentTime += process[shortest].ioBurstTime - 1;
            prev = currentTime;
        }
        currentTime++;
    }
